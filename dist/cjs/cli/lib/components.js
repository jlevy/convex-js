...
async function runCodegen(ctx, deploymentSelection, options) {
  ...
  if (options.offline || options.systemUdfs) {
    if (options.offline) {
      ...
      if (componentDir.kind === "ok" && componentDir.component.definitionPath && !componentDir.component.isRootWithoutConfig) {
        (0, import_log.logMessage)(
          import_chalk.chalkStderr.yellow(
    ...
    if (options.offline) {
      (0, import_log.logFinishedStep)("Types generated successfully (offline mode).");
    }
  } else {
